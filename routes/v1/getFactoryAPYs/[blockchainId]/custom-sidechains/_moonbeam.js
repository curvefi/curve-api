...
const networkSettings = {
  web3,
  multicall2Address: configs.moonbeam.multicall2Address,
};

export default async ({ version }) => {
  const config = configs.moonbeam;

  const poolData = (await getAllCurvePoolsData(['moonbeam'])).filter(({ registryId }) => (
    version === 'crypto' ?
  ...
  await Promise.all(
    poolData.map(async (pool, index) => {
      ...
      if (isCryptoPool) {
        ...
        // Moonbeam past block calls always fail, so don't bother making them and use fallback values straight away
        let vPriceOldFetchFailed = true;
        let vPriceOldFetch = 1 * (10 ** 18)
