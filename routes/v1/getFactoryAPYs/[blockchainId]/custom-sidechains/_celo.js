...
const networkSettings = {
  web3,
  multicall2Address: configs.celo.multicall2Address,
};

export default async ({ version }) => {
  const config = configs.celo;

  const poolData = (await getAllCurvePoolsData(['celo'])).filter(({ registryId }) => (
    version === 'crypto' ?
  ...
  await Promise.all(
    poolData.map(async (pool, index) => {
      ...
      if (isCryptoPool) {
        ...
        // Celo past block calls always fail, so don't bother making them and use fallback values straight away
